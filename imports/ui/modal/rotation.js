...
Template.rotationModalContent.helpers({
	rotation() {
		return _.find(this.events, { tag: 'rotation' });
	},

	vols() {
		return _.filter(this.events, evt => evt.tag === 'vol' || evt.tag === 'mep');
	},

	equipage() {
		const equipage = _.reduce(this.events, (crew, evt) => {
	...
	sv() {
		const vol = _.find(this.events, evt => evt.tag === 'vol' || evt.tag === 'mep');
		return vol ? vol.sv : {};
	}
});
...
Template.volsTable.helpers({
	showTable() {
		return this.vols && this.vols.length;
	},

	remarks() {
		return _.chain(this.vols)
	...
	airports() {
		return _.chain(_.map(this.vols, 'from').concat(_.map(this.vols, 'to')))
			.uniq()
			.without('ORY', 'CDG')
			.map(code => Airports.findOne({ iata: code }) || _.find(App.gares, { code }))
			.filter(_.identity)
			.value();
	}
});
